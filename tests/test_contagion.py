...
def test_infection_is_not_infectious_until_the_next_day():
    ...
    assert phenomenon.edge_probability(
        Edge(2, 1, "spouse", "Communal Sharing", 0.9, 0.9, 0.9, 0.9), state[2], state[1], day=1
    ) > 0.0

    phenomenon.end_of_day(graph, state, day=1)
...
def test_dead_residents_do_not_recover():
    ...
    state = phenomenon.init_state(graph)
    events = phenomenon.end_of_day(graph, state, day=1)
    assert events == []
